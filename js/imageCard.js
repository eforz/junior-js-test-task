...
// нужно переделать переменные, айдишник изображения
...
window.addEventListener("click", (event) => {
    if (event.target.hasAttribute('data-unfavorite')) {
        ...
        if (!imageInFavorites) {
            const photoHTML = `
                        <div class="accordion__content_img-wrapper" data-title="${imageInfo.title}">
                            <img class="accordion__content_img" src="${imageInfo.imgSrc}" alt="${imageInfo.title}" data-title="${imageInfo.title}" data-imageId="${imageInfo.id}" data-modal="#modal_1">
    ...
    if (event.target.hasAttribute('data-favorite')) {
        const image = event.target.closest(".accordion__content_img-wrapper")
        const starFavorite = image.querySelector("[data-favorite]")
        const starUnfavorite = image.querySelector("[data-unfavorite]")
        ...
        starFavorite.classList.remove('active')
        starUnfavorite.classList.add('active')

        if (imageInFavorites) {
            const ImgParent = imageInFavorites.closest(".accordion__content_img-wrapper")
            ImgParent.remove()
            toggleFavoritesStatus()
        }        
    }
    ...
    if (event.target.hasAttribute('data-from-favorite')) {
        ...
        const catalogImageWrapper = catalog.querySelector(`[data-imageId="${imageInfo.id}"]`) 
